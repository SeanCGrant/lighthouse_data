...
enc = 72
partition = 1  # 5.4 (1)
dps = 'dps' # 'rdps' for rdps or 'dps' for adps
...
for currentJob, currentVal in jobCodes2.items():
    # Initiate table
    conn = sqlite3.connect('shiva_adps_52.sqlite')
    cur = conn.cursor()

    cur.execute('''
    CREATE TABLE IF NOT EXISTS "{}" (name TEXT, adps NUMERIC, rdps NUMERIC, dps NUMERIC, duration NUMERIC,
        ...
        if val == 1:
            ...
            while js['hasMorePages']:
                ...
                serviceurl = 'https://www.fflogs.com:443/v1/rankings/encounter/{}?' \
                             'metric={}&partition={}&spec={}&filter=1.{}.1&page={}&api_key='.format(enc, dps,
                                                                                partition, currentVal, val, pageNum)
                url = serviceurl + api_key
                # try to get the json, and repeat (with intermission) until success
                ...
                for rank in js['rankings']:
                    ...
                    cur.execute('''SELECT adps FROM "{}" WHERE (name == ? AND server == ?)'''
                                .format(currentJob.replace("'", "''")), (rank['name'], rank['serverName']))
                    rdps = cur.fetchone()

                    if (rdps is None) or (rank['name'] == 'Anonymous'):
                        cur.execute(
                            '''INSERT INTO "{}" (name, adps, rdps, dps, duration, region, server, report, "{}")
                             VALUES (?,?,?,?,?,?,?,?,?)'''
                        ...
                        cur.execute('''UPDATE "{}" SET adps = ?, rdps = ?, dps = ?, duration = ?, report = ?, {} = ?
            ...
            while js['hasMorePages']:
                pageNum += 1
                serviceurl = 'https://www.fflogs.com:443/v1/rankings/encounter/{}?' \
                             'metric={}&partition={}&spec={}&filter=1.{}.0&page={}&api_key='.format(enc, dps,
                                                                                partition, currentVal, val, pageNum)
                url = serviceurl + api_key
                # try to get the json, and repeat (with intermission) until success
                invalidReturn = True  # invalid until proven otherwise
                ...
                for rank in js['rankings']:
                    ...
                    rdps = cur.execute('''SELECT adps FROM "{}" WHERE (name == ? AND server == ?)'''
                                       .format(currentJob.replace("'", "''")), (rank['name'], rank['serverName']))
                    rdps = cur.fetchone()
            ...
            while js['hasMorePages']:
                ...
                for rank in js['rankings']:
                    cur.execute('''UPDATE "{}" SET "{}" == ? WHERE (adps == ? AND name == ?)'''
                                .format(currentJob.replace("'", "''"), job.replace("'", "''")),
